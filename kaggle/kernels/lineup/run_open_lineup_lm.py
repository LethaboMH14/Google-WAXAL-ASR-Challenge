"""Kaggle GPU kernel: the all-ungated lineup WITH KenLM shallow fusion.
...
KENLM SHALLOW FUSION IS ON HERE — this is the stack on top of the no-LM open lineup.
Fusion measured +0.0066 on a real submission (0.7065 -> 0.7131) on the previous lineup, and the
mechanism it fixes is still unclaimed here: word-boundary loss. Our no-LM open lineup glues 2.30%
of its tokens into their neighbour, against 1.35% for the with-LM run of the old lineup and 2.48%
for its no-LM twin. Each glue costs two word errors under jiwer — one substitution for the fused
token, one deletion for the word that vanished — while CER barely moves, which is why it is worth
more than it looks. A word-level LM in beam search is exactly the thing that cannot emit a
non-word, so it is the targeted fix rather than a general-purpose dial.

All three checkpoints are CTC, so pyctcdecode applies to every language — unlike the whisper-sna
lineup, where fusion could only have touched two of the three.

Single-variable against whatever the no-LM open lineup scores: same checkpoints, same routing, same
period setting, LM on instead of off.

The corpus is built INLINE by running 00_build_lm_corpus.py in this session rather than mounting
stage 0's output. That mount has silently produced nothing once before — 03_decode_and_submit.py's
own comments record that both scored submissions on 1 Aug were decoded without a real LM because of
it. Building in-process removes the cross-kernel attach as a failure mode.

Expect this to run considerably longer than the ~50 min no-LM pass: corpus build and KenLM compile
are CPU work inside a GPU-billed session, and the alpha/beta sweep is 18 decode passes per language
before the real decode starts.
"""
...
print("\n=== STAGE 0: building the LM corpus in this session ===")
...
env["WAXAL_LM_CORPUS_DIR"] = str(LM_CORPUS_DIR)  # WAXAL_NO_LM deliberately unset
...
for name in ("submission_03_openlineup-lm_lm_phase1.csv", "submission_03_openlineup-lm_lm_phase2.csv"):
    csv = WORKING / name
    if csv.exists():
